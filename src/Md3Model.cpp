    ...
    _scale (1.0f), _name (filename)
{
  ...
  // Read frames.
  //Md3Frame_t frame; -- Moved to Global... (promotion much!?)
...
//----------------------------------------------------------------------------
//
// Awesome gets for collision detection stuffs.
//
//----------------------------------------------------------------------------
...
Md3Model::getLocalOrigin()
{
	// if there is a problem... delete it :)
	Vectormath::Aos::Vector3 origin;
	origin.setX(frame.local_origin[0]);
	origin.setY(frame.local_origin[1]);
	origin.setZ(frame.local_origin[2]);
	return origin;
}
